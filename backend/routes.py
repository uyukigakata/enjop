...
def analyze_images():
    try:
        ...
        openai_response = openai.ChatCompletion.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "投稿予定の文字列（ないかもしれない）と、動画・画像（一フレーム分の画像しかないときは画像、それ以外は動画）の分析結果・音声の文字起こしの結果について、添付された日本の法律についてのJsonファイルから、違反していそうな法律のID・法律名・今回の結果について法律の違反確率を1~10段階評価・なぜそう思ったか・その他のリスクについて、以下の様式にしたがってレスポンスを返してください。なお、レスポンスは日本語で返すこと。改行文字、エスケープ文字は含まないように返してください。"},
                {"role": "system", "content": """
                {"laws": [ { "law_id": "法律のID", # int "law_name": "法律名",  # str "law_risk_level": "法律の違反確率", # int(0~10)  "law_reason": "なぜそう思ったか" #str } ],  "comment": "その他のリスクについて", #str "rating": "1~10段階評価によるリスク評価" #int(0~10)} """},
        ...
        try:
            ...
            openai_content = json.loads(response_text)
        except json.JSONDecodeError as e:
            print(f"JSONデコードエラー: {e}")
            openai_content = {"error": "OpenAIレスポンスのフォーマットが不正"}
